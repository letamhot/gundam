...
<div id="comment-{{ $comment->getKey() }}" class="media">
    @if($comment->commenter->provider_id == null )
        <img class="mr-3" src="{{asset(Storage::url($comment->commenter->avatar))}}" alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar" style="border-radius:50%; width:60px; height:60px">
    @else
